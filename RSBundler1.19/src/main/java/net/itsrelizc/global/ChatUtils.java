...
public class ChatUtils {
	...
	public static void sayAsPlayer(Player player, TextComponent message) {
		Profile p = Profile.findByOwner(player);
		TextComponent s = new TextComponent(Rank.findByPermission(p.permission).displayName + " " + player.getDisplayName() + "§7: §r");
		s.addExtra(message);
		Bukkit.getServer().spigot().broadcast(s);
	}
	...
	public static void attachCommand(TextComponent component, String command) {
		component.setClickEvent(new ClickEvent(ClickEvent.Action.RUN_COMMAND, "/" + command));
	}
