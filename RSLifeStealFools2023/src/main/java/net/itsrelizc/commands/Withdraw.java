...
public class Withdraw implements CommandExecutor {
	...
	public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (!isEmpty) {
        	boolean haveHeart = false;
            for (ItemStack item : player.getInventory().getContents()) {
                if(item.getItemMeta().getDisplayName() == "§cHeart" && item.getAmount() < 64) {
                	haveHeart = true;
                    break;
                }
            }
            if (! haveHeart) {
            	ChatUtils.systemMessage(player, "§c§lHEARTS", "§cYou need more inventory space to withdraw a heart!");
            	return true;
            }
        }
		...
		ItemStack heart = Items.getHeartItem();
		...
		Integer h = ((int) player.getMaxHealth()) / 2;
		ChatUtils.systemMessage(player, "§c§lHEARTS", "§aNow you have §c" + h + " ❤§a, which gives you §c" + h * 2 + " ❤ Max Health§a.");
