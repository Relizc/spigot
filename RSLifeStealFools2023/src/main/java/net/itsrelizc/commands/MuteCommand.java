    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
    	if (sender instanceof Player) {
    		...
    		if (PlayerManager.ranks.get(p).level <= PlayerManager.ranks.get(c).level) {
    			ChatUtils.systemMessage(p, "§c§lCOMMAND", "§cYou are not allowed to mute this player!");
    			return true;
    		}
    	}
    	...
    	MuteUtils.createMute(player, "\u00a7bMuted by an operator: \u00a7e" + reason, (long) 86400);
    	ChatUtils.systemMessage(sender, "§e§lBANS", "§aSucessfully muted player " + player.getDisplayName() + " §7(UUID: " + player.getUniqueId().toString() + ")");
