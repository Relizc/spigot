...
public class TPAccept implements CommandExecutor {
	...
	public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
		...
		if (Items.countTicket(target) <= 0) {
			TPACommand.tpas.remove(target);
			ChatUtils.systemMessage(target, "§d§lTELEPORT", "§cWe cancelled your teleport because your teleport ticket is gone!");
			ChatUtils.systemMessage(player, "§d§lTELEPORT", "§cThe teleport request is cancelled because the teleporter don't have enough tickets!");
			return true;
		}
