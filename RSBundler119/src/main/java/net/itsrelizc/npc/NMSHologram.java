...
public class NMSHologram extends CraftArmorStand implements Listener {
...
//		holo.f(tag);
	...
	public void sendSpawnPacket(final Player player) {
		...
		PacketPlayOutSpawnEntity a = new PacketPlayOutSpawnEntity(this.getHandle(), 78);
		
		DataWatcher dw = new DataWatcher(this.getHandle());
//		dw.a(null)
//		dw.a(2, "L bozo");
//		dw.a(3, 1);
		final PacketPlayOutEntityMetadata b = new PacketPlayOutEntityMetadata(this.getEntityId(), dw, false);
		...
		Bukkit.getScheduler().runTaskLater(Bukkit.getPluginManager().getPlugin("RSBundler"), new Runnable () {
			...
			@Override
			public void run() {
				((CraftPlayer) player).getHandle().b.a(b);
			}
	...
	public NMSHologram(World world, double d0, double d1, double d2) {
		super(world.getCraftServer(), null);
	}
