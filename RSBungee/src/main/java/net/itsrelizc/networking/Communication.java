...
import net.itsrelizc.networking.executors.ConnectPlayer;
import net.itsrelizc.networking.executors.CreateServer;
import net.itsrelizc.networking.executors.KickPlayer;
import net.itsrelizc.networking.executors.RemoveServer;
			...
			public void run() {
				try {
					...
					for (CommunicationInput input : inp.getAllPackets()) {
						...
						self.attachExecutor(CommunicationType.BUNGEE_CREATE_SERVER, new CreateServer(input));
						self.attachExecutor(CommunicationType.BUNGEE_REMOVE_SERVER, new RemoveServer(input));
						self.attachExecutor(CommunicationType.PLAYER_MOVE, new ConnectPlayer(input));
						self.attachExecutor(CommunicationType.BUNGEE_STOP, new Runnable() {@Override public void run() {ProxyServer.getInstance().stop();}});
